...
SUB NAV
  ...
  X = ENTERPRISE(1)
  Y = ENTERPRISE(2)
  PRINT "TRAVERSE THROUGH FOLLOWING SECTORS:"
  PRINT 0 " : ("+ FORMAT$(ENTERPRISE(1))  + "," + FORMAT$(ENTERPRISE(2)) + ")" 
  FOR I = 1 TO SPEED

    X = CINT(ENTERPRISE(1) + I * COS(DIRECTION))
    Y = CINT(ENTERPRISE(2) + I * SIN(DIRECTION))
    
    PRINT I ": (" + FORMAT$(X)  + "," + FORMAT$(Y) + ")";
    
    IF GALAXY(X,Y) <> 0 THEN PRINT "THIS SECTOR WILL BLOCK MOVE! BECAUSE OF "+ FORMAT$(GALAXY(X,Y)) ; 
